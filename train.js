...
function readFile(file) {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(`data/${file}`, "utf8", (err) => {
      if (err){
        reject(err)
      } else{ 
...
(async () => {
  ...
  filenames.map(async file => { // 75 times
    ...
    if(allData.length === totalNumDataFiles){
      ...
      console.log('punches ',punches, 'hadokens', hadokens, 'uppercuts', uppercuts)
    }
  })
})()
...
const format = (allData) => {
  ...
  console.log(justFeatures[0].length)
  console.log(justFeatures[1].length)
  console.log(justFeatures[2].length)
